...
fn read_trim(path: &str) -> String { fs::read_to_string(path).unwrap_or_default().trim().to_string() }

fn os_pretty_name() -> String {
    let raw = read_trim("/etc/os-release");
    raw.lines().find_map(|line| line.strip_prefix("PRETTY_NAME=")).map(|v| v.trim_matches('"').to_string()).unwrap_or_else(|| "Linux".into())
}
...
fn cpu_model() -> String {
    ...
    for key in ["model name", "Hardware", "Processor"] {
        if let Some(line) = raw.lines().find(|l| l.starts_with(key)) {
            if let Some((_, value)) = line.split_once(':') { return value.trim().to_string(); }
        }
    }
...
fn memory_bytes() -> u64 {
    ...
    raw.lines().find_map(|line| {
        ...
        if p.next()? == "MemTotal:" { p.next()?.parse::<u64>().ok().map(|kb| kb * 1024) } else { None }
    }).unwrap_or(0)
}

fn uptime_seconds() -> u64 {
    read_trim("/proc/uptime").split_whitespace().next().and_then(|v| v.split('.').next()).and_then(|v| v.parse().ok()).unwrap_or(0)
}
