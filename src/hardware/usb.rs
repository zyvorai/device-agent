...
fn read(path: impl AsRef<Path>) -> Option<String> { fs::read_to_string(path).ok().map(|v| v.trim().to_string()).filter(|v| !v.is_empty()) }

pub fn collect() -> Vec<UsbDevice> {
    let mut out = Vec::new();
    let Ok(entries) = fs::read_dir("/sys/bus/usb/devices") else { return out; };
    for entry in entries.flatten() {
        let base = entry.path();
        if !base.join("idVendor").exists() { continue; }
        out.push(UsbDevice {
