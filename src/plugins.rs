...
pub async fn sample(cfg: &Config, manifest: &PluginManifest) -> PluginResult {
    ...
    let result = timeout(
        Duration::from_secs(cfg.plugins.timeout_seconds),
        cmd.output(),
    )
    .await;
    match result {
        Ok(Ok(output)) if output.status.success() => {
            let parsed = serde_json::from_slice(&output.stdout).unwrap_or_else(
                |_| serde_json::json!({"raw": String::from_utf8_lossy(&output.stdout)}),
            );
            PluginResult {
        ...
        Err(_) => PluginResult {
            name: manifest.name.clone(),
            ok: false,
            output: serde_json::json!({"error": "plugin timeout"}),
        },
    }
}
